...
// kanban basic
export const KanbanBasic: React.FC<any> = ({ containerClass, data, ...props }) => {
  ...
  return (
    <DragDropContext
      onDragEnd={(result: any) => {
        ...
        const workValue: any = items.slice()
        ...
        {(provided: any) => (
          ...
          >
            {items.map((item: any, index: number) => (
              <DroppableList key={item.id} index={index} {...item} />
            ))}
...
// kanban custom
export const KanbanCustom: React.FC<any> = ({ containerClass, data, ...props }) => {
  const [items, setItems] = useState<any>([])
  const [groups, setGroups] = useState<any>({})
...
// kanban drop list
const DroppableList: React.FC<any> = ({ id, items, label, theme }) => {
  return (
    <Droppable droppableId={id}>
      {(provided: any) => (
        <div className="kanban-item-wrap" {...provided.droppableProps} ref={provided.innerRef}>
